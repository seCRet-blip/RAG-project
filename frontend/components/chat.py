...
def page_style() -> str:
    ...
    .answer-box { background: #f0f4f8; padding: 1rem; border-radius: 8px; margin: 1rem 0; }
    .source-card { border-left: 3px solid #3b82f6; padding: 0.5rem 1rem; margin: 0.5rem 0; background: #fafafa; }
    .preview { color: #555; font-size: 0.9rem; }
...
def chat_ui():
    return Form(
        Input(name="message", placeholder="e.g. What's the difference between ENTRYPOINT and CMD?", required=True),
...
def source_card(source: dict) -> Div:
    ...
    return Div(
        ...
        P(source.get("preview", ""), cls="preview"),
        cls="source-card",
    )
